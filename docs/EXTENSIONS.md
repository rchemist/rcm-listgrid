...
| **Customize a 0.5.x list filter input** | `registerFilterRenderer(type, Component)`; controlled props are `{field,value,onChange(value, operator?)}`. A valid renderer operator has highest precedence, followed by validated `withFilter({operator})`, then the value/type mapping in `searchConditionFor(field, value?, rendererOperator?)`. | `packages/react/src/registry/filter-renderer-registry.tsx` |
| **Customize a 0.5.x list cell** | `FieldListConfig.format` for a string, `registerListCellRenderer(type, Component)` for a React view, or an explicit `{name,label,render(row)}` column. | `packages/react/src/components/ViewListGrid.tsx` |
| **Add row popup navigation** | `ViewListGrid.openInNewWindow` with host-owned `getUrl(row)` and optional `showFilter(row)`; do not add a second clickable cell in a custom renderer. | `packages/react/src/components/ViewListGrid.tsx` |
...
| **Re-apply host-owned filter state without remounting** (host owns its own FilterBar / URL state and injects via `options.filters`) | `options.filtersKey` — pass a value derived from the host filter state (e.g. `JSON.stringify(filterState)` or a hash). When it changes the engine clears prior filters/sort, re-runs `options.filters`, and refetches from page 0 (no `key`-prop remount). Opt-in: omit it and behavior is unchanged. **Don't** drive filter changes by remounting `<ViewListGridWrapper key={…}>` — that tears down state and forces a cold re-init. | `components/list/hooks/useListGridLogic.ts` |
