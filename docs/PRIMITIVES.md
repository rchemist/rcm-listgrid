# @rcm/listgrid — Primitive Catalog
...
- **Import**: `import '@rcm/listgrid/styles.css'` (or `./styles/primitives.css` alone)
...
- **Cascade order**: `tokens → primitives → layouts → components → base`. Hosts override by writing CSS after `@rcm/listgrid/styles.css`.
...
### Token overrides
Override design tokens in your own CSS, loaded **after** `@rcm/listgrid/styles.css`:
...
- [`DECISIONS.md`](../DECISIONS.md) — design decision log (entries #57, #60, #61 cover the CSS architecture)
