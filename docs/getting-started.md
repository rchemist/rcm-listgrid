# Getting started with `@rcm/listgrid`
...
- **React**: `>= 18`. Declared in `peerDependencies`. React 19 works — the library avoids 19-only syntax on purpose ([DECISIONS #3](../DECISIONS.md)).
...
Until the first npm publish, pin the release repo tag in `package.json`:
...
(If you want finer control, import the individual layers — `@rcm/listgrid/styles/tokens.css`, `.../primitives.css`, `.../layouts.css`, `.../components.css`, `.../base.css` — in that order.)
...
`@rcm/listgrid/next` is a thin binding to `next/navigation` + `nuqs`. For React Router / Remix / TanStack Router, write your own `RouterServices` and `UrlStateServices` implementations — their contracts are exported as types:

```ts
import type { RouterServices, UrlStateServices } from '@rcm/listgrid';
```
...
```
[@rcm/listgrid] UI component "SelectBox" missing from UIProvider.
```
...
**(c) Reference the original extraction source.** The library was extracted from a production project (see [`docs/ROADMAP.md`](./ROADMAP.md) Stage 1). That project's `UIProvider` values are a working reference if you need inspiration — but they depend on that app's own design tokens, so they are not directly reusable.

> **No official adapter ships yet.** Stage 6 of the roadmap lists a reference adapter as follow-up work. Until then, every adopter writes their own map. This is the biggest single integration cost.
...
```
[@rcm/listgrid] Compound "Table.Th" missing on host component.
```
...
Default is `any` — existing code compiles without change. Opt in entity-by-entity when you want the safety. The same `<T>` parameter flows through `FieldRenderParameters<T, TValue>`, `FilterRenderParameters<T, TValue>`, `ViewRenderProps<T>`, and `ViewValueProps<T>` (Task F/G in [`CHANGELOG.md`](../CHANGELOG.md)).
...
- **Tokens**: override CSS custom properties in your own stylesheet loaded **after** `@rcm/listgrid/styles.css`. See [`src/listgrid/styles/tokens.css`](../src/listgrid/styles/tokens.css) for the full ~50-token list.
...
**`error TS2305: Module '"@rcm/listgrid"' has no exported member 'X'`.** Something you are importing does not exist at the top-level entry. Check [`src/listgrid/index.ts`](../src/listgrid/index.ts) — it is the only surface. Field files (`StringField`, …), configuration (`EntityForm`, `ListGrid`), and providers (`AuthProvider`, `UIProvider`, …) are all re-exported from there.

**`[@rcm/listgrid] useUI must be called within a <UIProvider>`.** You are rendering a listgrid component outside the provider tree, or you imported a field from a test setup that skipped providers. Wrap the tree, or use a test wrapper that mounts all five providers.

**`[@rcm/listgrid] UI component "X" missing from UIProvider`.** Your `components` prop to `UIProvider` doesn't include that primitive. Add it — see the table in § 4.

**`[@rcm/listgrid] ApiClient called but no ApiClient has been configured`.** You rendered a list / form before `configureApiClient(...)` ran. Make sure the `configure*` calls live at module scope or run before any component mounts (a `bootstrap.ts` imported at the top of your entry file works well).
...
**Dates / numbers look localised incorrectly.** Override the formatter functions the library uses internally (`fDate`, `fDateTime`, `formatPrice`) — or import your own equivalents from the `@rcm/listgrid` misc barrel and wrap field render logic.
...
- [`docs/ROADMAP.md`](./ROADMAP.md) — the Stage-based extraction history and remaining Stage 6 work.
- [`DECISIONS.md`](../DECISIONS.md) — 70+ design decisions, each with a `Why`. Read before opening a PR.
