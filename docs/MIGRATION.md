...
This document walks you from any `0.1.0-alpha.x` release of `@rcm/listgrid` to **`v0.2.0`**. It expands the entries in [`CHANGELOG.md`](../CHANGELOG.md) into before/after code samples and diagnostic messages so you can grep for TypeScript errors and find the relevant fix directly.

---

## v0.2.0 — summary

`v0.2.0` is the first **public minor** after the alpha line (alpha.1 → alpha.49). It folds two things into one version bump:
...
In practice, migration is cheap: on the original production project the library was extracted from, the type-check went from `alpha.47` → `v0.2.0` with **zero source changes**. TypeScript 5.x is lenient enough about `unknown === 'literal'` comparisons that most `Map<string, any>` → `Map<string, unknown>` call sites compile unchanged.
...
## 1. Upgrade the package pin

Until the first npm publish, update your `package.json` to the `v0.2.0` tag of the release repo:

```diff
  "dependencies": {
-   "@rcm/listgrid": "github:rchemist/rchemist-rcm-listgrid-release#v0.1.0-alpha.47"
...
Once `@rcm/listgrid` is published to npm, the pin becomes `"@rcm/listgrid": "^0.2.0"` and `npm install @rcm/listgrid@^0.2.0` works as normal.
...
**Why.** The value bag is genuinely heterogenous — strings, numbers, booleans, objects, arrays. `any` silently suppressed errors for the caller; `unknown` forces the call site to declare what it expects. TypeScript 5.x still lets `raw === 'literal'` comparisons compile without narrowing, so most callers need no change. Full rationale: [DECISIONS #75](../DECISIONS.md).
...
**Why.** The actual `HeaderActionButtons` JSX has been emitting `rcm-button` + `data-variant` / `data-color` primitive markup for several alphas — the slot was already a no-op. The v0.2.0 cleanup just deletes the dead surface. See [DECISIONS #61 / #75](../DECISIONS.md).
...
```css
/* in your own stylesheet, loaded after @rcm/listgrid/styles.css */
...
```
error TS2305: Module '"@rcm/listgrid"' has no exported member 'getColorIndicator'.
```
...
```diff
- import { getColorIndicator, getIndicatorTone } from '@rcm/listgrid';
+ import { getIndicatorTone } from '@rcm/listgrid';
...
None of the following require action. They are new opt-in type narrowing released in alpha.48 (Task F) and alpha.49 (Task G), now part of `v0.2.0`. Adopt incrementally where it pays off.

### Task E — `EntityForm<T>` key narrowing
...
### Task E — `FormField<TSelf, TValue, TForm>`
...
### Task F — `FieldRenderParameters<T, TValue>`
...
### Task G — `ViewRenderProps<TForm>` / `ViewValueProps<TForm>`
...
If you hit an edge the CHANGELOG doesn't cover, check [`DECISIONS.md`](../DECISIONS.md) entry #75 (the full v0.2.0 design record) or open an issue on the source repo.
