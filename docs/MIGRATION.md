...
| 39 | SearchForm군: `withFilter`(20)/`withSort`(5)/`withPage`(16)/`withPageSize`(25)/`quickSearch` | `SearchForm` 존치(wire format, 불변 빌더) | 무변경 | `SearchForm` 클래스 자체와 그 빌더 메서드는 이름·시그니처 불변, import 경로만 `/schema`로 바뀐다(§2 서브패스 제거 절 참조). **주의**: `SearchForm.withFilter(...)`(AND 필터 추가)와 신규 필드 레벨 `FormField.withFilter(config)`(고급검색 참여 선언, #38행)는 **동명이지만 다른 클래스의 다른 메서드**다 — 혼동 주의. |
