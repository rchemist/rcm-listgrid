...
- 목록 조회가 실패했을 때 이전 조회 결과와 이전 검색조건이 그대로 남던 문제를 수정
  (gjcu-academic-backend, 관리자 재학생 목록). `fetchData` 의 에러 분기는
  메시지만 노출하고 `setRows` 와 URL/sessionStorage 동기화를 성공 분기에서만
  수행했기 때문에, 조회에 실패하면 화면에는 직전 필터의 행이 그대로 남고
  URL 에도 직전 필터가 남아 새로고침 시 되살아났다.

  실제 증상: 개인정보 검색 제한(`PERSONAL_SEARCH_CONDITION_REQUIRED`, 400)이
  걸린 목록에서 고급검색 **초기화** 를 누르면 "조건 없는 조회" 가 서버에서
  거부되는데, 화면은 초기화 이전의 필터 결과를 계속 보여주고 새로고침하면
  지웠던 검색조건이 다시 적용됐다. 이제 실패 시에도 요청했던 SearchForm 을
  URL/sessionStorage 에 반영하고 결과 집합을 비워, 화면의 검색조건과 목록이
  어긋나지 않는다.
