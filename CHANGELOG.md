...
- 행 새 창(팝업) 동작을 0.2.x 계약으로 복원했다 (egov-cms#84 W6): 기본 1200×800·화면
  가용 크기 상한·중앙 배치, 경로별 창 크기 localStorage 기억(1초 추적), 창 이름 entity_{id}.
- 목록이 `ENTITY_DELETED`/`ENTITY_SAVED` postMessage 를 받으면 자동 재조회한다
  (팝업에서의 삭제 → 부모 목록 갱신).
...
- 통합 검색 패널에 0.2.x의 '검색 필드 선택 N/N' FieldSelector 섹션(전체 선택/해제·필드 칩·접기)과
  그리드↔리스트 레이아웃 토글을 복원했다 (egov-cms#84 R7).
- 목록 select 뱃지 전용 `.rcm-list-badge`(4px 라운드 사각·색 보더+틴트 bg, gjcu host Badge 기하)를 추가했다.

### Fixed

- 헤더 셀 순서를 0.2.x와 같은 라벨→정렬→필터로 되돌리고, 정렬(IconSortDescending 계열 3종)·필터(IconFilter)
  글리프와 색(미정렬 상속·active #4680ff)을 0.2.x 실효값으로 복원했다.
- 퀵서치 돋보기/클리어 아이콘을 tabler 원본 path(stroke 1)로 교체했다.
- 통합 검색 카운트를 'N개 필드' 형식으로, 푸터 닫기/초기화/검색 버튼에 아이콘을 복원했다.
- 새 창 버튼을 gjcu 실효값(32x36·secondary 보라 아웃라인·16px 아이콘)으로 맞췄다.
...
- `ViewListGrid`에 라벨 재정의 가능한 빈 목록 상태와 opt-in 내림차순 행 번호 열을 추가했다.
- 컬럼 필터·정렬 상태를 나타내는 인라인 SVG 아이콘과 필터/고급검색 스타일링용
  `data-*` 구조 훅을 추가했다.
- `ViewEntityForm`에 필드 오류를 실시간으로 집계하고 해당 필드로 이동하는 접이식 오류 요약을
  추가했다.

### Fixed

- 필수 값 검증 문구가 목적격 조사와 이중 공백 대신 받침에 맞는 주제격 조사 `은/는`과 단일
  공백을 사용하도록 수정했다.
