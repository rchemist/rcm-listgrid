...
이 파일은 `@rcm/listgrid` 의 공개된 변경 이력을 기록합니다.

## [0.2.0] - 2026-04-XX

### Summary

"의도된 any 중 공개 API 청소 완료" 마일스톤. alpha.40~alpha.49 누적 리팩터 (framework-free + CSS primitive + exactOptionalPropertyTypes + TSelf/TForm/TValue 제네릭 + parse unknown) 을 공식화하고, 누적된 `@deprecated` / "TODO: remove in v0.2" 항목을 정리.

설계 문서: `docs/V020_BREAKING_DESIGN.md`.
...
DECISIONS #61 유예 사항 실행. `HeaderActionButtons` JSX 가 이미 `rcm-button` + `data-variant`/`data-color` primitive 를 직접 사용하므로 slot 은 이미 비활성.
...
#### Task G — parse 제네릭 + ViewRenderProps 제네릭
...
**gjcu 실측 영향 (2026-04-19)**:

- A-1 attributes: `as` cast 이미 자리잡은 패턴 + `unknown === 'literal'` 비교는 TS 5.x 허용 → **0 errors**
- A-2/A-3/B-4/B-5/B-6: **0 errors**

### Task F/G alpha tag 상태

- alpha.48 (Task F) / alpha.49 (Task G) 는 v0.2.0 의 구성 요소 (interim releases).
