...
**Created**: 2026-07-10
**Status**: active · **Phase GX(0.4 프레임워크 정합·GA-blocking) 진행 — GX-1/2 ✅**(SearchForm wire+mock 백엔드 rcm-backend-framework 0.1.0 정합·withFilter 복원·2254u/E2E 32 green). **Next up: GX-3**(/utils 이식·결정 확정)→GX-4/5→GA 게이트(CAP-28). 근거 [gap analysis](./analysis/2026-07-12/w7-post-seal-gap-analysis.md). 계수 49/57/186. P0/P1 publish=외부 승인 대기.
...
**Last updated**: 2026-07-13 (**GX-2 ✅** — apps/sample mock 백엔드+어댑터 에러파서를 프레임워크 0.1.0 응답/에러 계약에 정합: `searchEnvelope` 9필드 SearchResponse·404 RFC7807 ProblemDetail(어댑터 `detail??title??message`)·employee/collabo filters 배선·"Spring Page" 주석 정정. sonnet 위임→메인 authoritative: type-check clean·**full unit 2254 pass**(+3·0회귀)·**full E2E 32/32**(+2 backend-contract·404 라이브 실증). §Needs Review +1(mock 5/24 조건타입). 상세 [archive #GX-2](./progress-archive/phase-gx-tasks.md#gx-2). 다음=GX-3(/utils).)
