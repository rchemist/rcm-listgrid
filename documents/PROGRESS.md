...
**Last updated**: 2026-07-13 (**TB-4 완료** — CRUD 균일화+bulk delete+revision passthrough: employee/collabo/org/staff=makeCollectionHandlers 통합·major=DELETE 추가, 5 결함엔티티 멀티행 bulk delete 실결함 수정. revisionEntityName passthrough(낙관락無). delegate(sonnet done_with_deviations·bulk DELETE 204 fidelity=§Needs Review·TB-6 재판정). bulk-delete.test.ts 8 green·전량 2466 무회귀. **Next up=TB-5(M2O round-trip).**)
...
- **다음(Phase TB — 활성·5/10 완료)** = 백엔드 테스트 Full Set(사용자 지시 2026-07-13). **TB-0~4 완료**(commit `cc40196`→`8e1d38b`): 계약 확정([tb0](./analysis/2026-07-13/tb0-contract-confirmation.md))·필터 엔진(24 조건타입+NOT+nested subFilters)·정렬/quickSearch/pagination·에러 route(6 팩토리+7 매핑)·CRUD 균일화+bulk delete 실결함 수정+revision passthrough. 규범=[recon](./analysis/2026-07-13/test-backend-recon.md)+[매칭/정렬 계약](./analysis/2026-07-13/tb-matching-semantics.md). **Next up=TB-5(M2O round-trip)~TB-9**. **위임 규율**: source-edit=sonnet delegate(brief=execution-grade·framework 인용)·메인이 authoritative 검증+commit. TB-1/2/3 파일겹침(store/crud-routes)→**순차**(병렬 아님). 전량 **2466 green**. 아키텍처=옵션 A(apps/sample mock 승격). **핵심**: framework 0.1.0 매칭 계약=citable(FilterDispatcher/SearchRequestPlanner·NOT=`!(and)`)·M2O=중첩`{id,title}`(save→`<name>Id`)·excel/upload=백엔드 API 아님·**bulk DELETE 204 fidelity=§Needs Review(TB-6 재판정)**.
...
**Next up**: **TB-5** (M2O/참조 round-trip). TB-0~4 완료. TB-5 파일=major toWire/fromWire·store fixtures. Proof=RV-R13 회귀(PUT body가 flatten `<name>Id`·중첩 아님 단언)+bare-id 참조해석 getOne. TB-6(계약 스위트·204 fidelity 재판정)은 TB-1~5 후.
