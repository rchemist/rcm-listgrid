...
|---|---|---|---|---|---|
| W5-1 | 필드 list/filter 선언 substrate (§5.1) | 신규 `packages/schema-core/src/field/list-config.ts`(타입) · `field/form-field.ts`(빌더 :163-245 인접) · `field/many-to-one-field.ts:45,63-67`(showInList→withList 위임) · `schema-core/src/index.ts`(배럴 +2) · `__tests__` | 신설 타입 `FieldListConfig {order?, label?, align?, width?, sortable?}`·`FieldFilterConfig {operator?, order?, label?}`(§5.1 그대로) · FormField `withList(config?: FieldListConfig \| false): this`+`getListConfig()`·`withFilter(config?: FieldFilterConfig \| false): this`+`getFilterConfig()` (in-place mutate+return this, §5.2) · `useListField()`→`this.withList()` 위임(showInList 필드 제거) | unit: with*/get* round-trip · `false`=명시제외 vs undefined=미선언 구분 · useListField()⟹getListConfig() truthy · 계수 /schema=184(≤190) | 마법 폴백·파생 정렬 금지(W5-2) · store 런타임 override 아님(선언-타임 인스턴스 필드 §5.2) · `FieldMetaOverride`에 list 키 추가 금지(별도 채널) |
| W5-2 | ViewListGrid 컬럼 파생 교체 + list-cell 레지스트리 (§7 CAP-19) | `packages/react/src/components/ViewListGrid.tsx:75-134`(deriveDefaultColumnNames/resolveColumns 교체) · 신규 `react/src/registry/list-cell-renderer-registry.tsx`(`field-renderer-registry.tsx:28-38` 패턴 복제) · `registry/xref-prefer-mapping-renderer.tsx:204-214`(중복→공유 파생) · `react/src/index.ts`(registerListCellRenderer +1) · apps/sample 리스트 페이지(필드 withList 이행) · `e2e/` | 컬럼 파생=getListConfig() truthy 필드 수집·정렬(config.order ?? field.order)·label/align/width 반영 · **마법 폴백 폐기**(withList 0 → 빈 컬럼+`console.warn`, §5.1 "첫 Listable 자동채택" 폐기) · `columns` prop escape hatch 유지 · sortable 헤더→`store.setSort` · 셀=`getListCellRenderer(type)` 조회(폴백 getDisplayValue/String, 현 bare `String()` :124/:132 대체) | unit: 파생(order/label/align/width·false 제외·0→빈+warn) · E2E: sample 폼 withList 선언→리스트 컬럼 렌더(College/Subject 재사용) | 첫-4-비숨김 폴백 유지 금지(§5.1 폐기) · 합성 컬럼 자동주입 금지 · `ViewEntityForm.tsx` 무관(별 hot-file) |
