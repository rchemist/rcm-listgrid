    ...
    return (async () => {
      ...
      if (readonly) {
        return <TextInput
          ...
          value={fDateTime(value)}></TextInput>
      } else {
        return <FlatPickrDateField type={'datetime'} limit={this.limit} range={this.range} {...await getInputRendererParameters(this, params)}/>
      }
    })();
  }
    ...
    if (this.range) {
      ...
      if (viewRaw) {
        ...
        if (isBlank(value)) {
          ...
          return Promise.resolve({result: <Tooltip label={`${fDateTime(props.item[this.name])}`}><span>{value}</span></Tooltip>});
        }
      }
    }
  }
