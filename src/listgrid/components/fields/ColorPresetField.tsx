...
async function renderColorListField(field: ColorPresetField, props: ViewListProps): Promise<ViewListResult> {
...
// 색상을 선택하는 버튼을 렌더링한다.
const ColorPresetFieldView = ({name, label, required = false, readonly = false, presets = [], ...props}: ColorPresetFieldProps) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Popover.Target>
          <button className={`btn ${getAdditionalColorClass(selectedColor)} ${getOppositeTextColorClass(selectedColor)}`} disabled={readonly} onClick={() => setIsOpen(true)}>
            색상 선택
          </button>
      </Popover.Target>
      <Popover.Dropdown>
