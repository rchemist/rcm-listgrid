...
const AvailableMappingEntityForm = (mapping: {
  ...
  label: string,
  requiredAvailable: boolean;
  filter?: FilterItem[] | ((entityForm: EntityForm, parentEntityForm?: EntityForm) => Promise<FilterItem[]>);
  parentEntityForm?: EntityForm;
}): EntityForm => {
      ...
      fields: [
        new StringField('mapping-available', 10000).withLabel('유효기간')
          ...
          .withOverrideRenderListItem(async (props: ViewListProps) => {
            return {
              result: <div className={'w-full flex items-center justify-center'}>
                {props.item['mapping-available'] === undefined ? '항상' :
