...
interface SelectFieldRendererProps {
  name: string;
  value: FieldValue;
  fetchedValue?: FieldValue;  // 서버에서 가져온 원본 값
...
export const SelectFieldRenderer: React.FC<SelectFieldRendererProps> = ({
  ...
  const availableOptions = useMemo(() => {
    ...
    return options.filter(opt =>
      opt.value === originalValue || originalOption.targets?.includes(opt.value as string)
    );
  ...
  const getReasonConfigForStatusChange = useCallback((targetStatus: FieldValue): StatusReason | null => {
    if (!reason || reason.length === 0) return null;

    // targets가 명시된 설정 찾기
    const specificConfig = reason.find(r =>
      r.targets && r.targets.includes(String(targetStatus))
    );
  ...
  const handleStatusChange = useCallback(async (changeReason?: string) => {
    ...
    try {
      ...
      if (immediateChangeProps?.onSubmit) {
        const result = await immediateChangeProps.onSubmit(
          entityForm,
          { targetValue: selectedValue, formData }
        );
        if (result === false) {
          setOpenBaseLoading(false);
          setIsChanging(false);
  ...
  // 상태 라벨 가져오기
  const getStatusLabel = useCallback((statusValue: FieldValue): string => {
  ...
  const handleSelectChange = useCallback((newValue: FieldValue) => {
