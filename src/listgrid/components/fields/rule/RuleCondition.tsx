    ...
    <div
      ...
      <div className={'form-input w-fit flex relative mt-[-20px] space-x-2 items-center'}>
        ...
        <div>
          <button className={'mt-1'} onClick={() => {
            clearCondition();
          }}><IconCircleX className="max-w-4"/></button>
        </div>
      ...
      <div className={'flex gap-3 items-center mt-3'}>
        ...
        <div className={'flex gap-3 items-center'}>
          <button className={'btn btn-secondary h-[36px]'} disabled={!selected} onClick={() => {
            const newFieldValues = [...fieldValues];
        ...
        return fieldValues.map((fieldValue, index) => {
          ...
          return <div key={index}>
                ...
                {function () {
                  ...
                  if (currentValueType === 'SINGLE') {
                    ...
                    if (field instanceof AbstractDateField) {
                      ...
                      return <div className={'w-full'}><RuleFieldRenderer field={cloned} onChange={(value) => {
                        if (Array.isArray(value) && value.length === 2) {
                          changeValue(index, [...value]);
                        }
                      }}></RuleFieldRenderer></div>
                    } else {
                      ...
                      return <div key={`${fieldValue.name}_${index}`} className={'flex justify-between'}>
                        <RuleFieldRenderer field={field1} onChange={(value) => {
                        changeValue(index, [...value]);
                      }}></RuleFieldRenderer> ~
                        <RuleFieldRenderer field={field2} onChange={(value) => {
                          changeValue(index, [...value]);
                        }}></RuleFieldRenderer>
                      </div>
            ...
            {fieldErrors?.has(index) &&
              <div className={'px-2'}><ViewFieldError errors={[fieldErrors.get(index)!]}/></div>}
            <div className={'px-2'}>
