...
export const RuleBasedFieldsView = (props: RuleBasedFieldViewProps) => {
  ...
  const [ruleConditions, setRuleConditions] = useState<Map<number, RuleConditionValue>>(new Map<number, RuleConditionValue>());

  const [fieldErrors, setFieldErrors] = useState<Map<number, Map<number, string>>>(new Map<number, Map<number, string>>());
  ...
  useEffect(() => {
    ...
    if (props.value instanceof Map) {
      ...
      const newMap = new Map<number, RuleConditionValue>(
        Object.entries(props.value).map(([key, value]) => [Number(key), value as RuleConditionValue])
      );
  ...
  function addNewRuleCondition() {
    if (condition !== undefined && targetRuleFieldEntityForm !== undefined) {
      ...
      const newRuleConditions: Map<number, RuleConditionValue> = new Map<number, RuleConditionValue>(ruleConditions);
  ...
  return <>
    <div className="rcm-rule-wrap">
      <div className={`rcm-rule-inner ${isButton ? 'rcm-rule-inner-button' : 'rcm-rule-inner-plain'}`}>
        ...
        <div className={'flex gap-3 items-center'}>
          {(props.entityForms.length > 1) && <div className={'min-w-[200px]'}>
              ...
              }} name={'chooseTargetEntityForm'}></SelectBox>
            </div>}
          ...
          <div>
            <button className={'btn btn-secondary h-[36px]'}
                    disabled={(condition === undefined || targetRuleFieldEntityForm === undefined)} onClick={() => {
              addNewRuleCondition();
            }}>추가
            </button>
          </div>
      ...
      {!isBlank(submitError) && <div className={'px-2 mt-5'}>
    ...
    {props.viewType === 'selector' &&
      <div className={'flex gap-3 items-center justify-center mt-5'}>
        ...
        <button className={'btn btn-secondary h-[34px]'} disabled={!filtered} onClick={() => {
          submit();
