...
export const ShowNotifications = ({error, color, timeout, showClose = false, onClick, ...props}: ListGridNotificationsProps): React.ReactNode => {
  ...
  useEffect(() => {
    ...
    // 중복을 제거한다.
    const uniqueMessages = messageItems.filter((item, index, self) =>
                ...
                <div className={`${textColor} w-full`}>
                  {uniqueMessages.map((item: MessageItem) => (
                    <div
                      key={item.id}
                      id={item.id}
                      className={'flex space-x-2 items-center font-semibold cursor-pointer hover:opacity-80'}
