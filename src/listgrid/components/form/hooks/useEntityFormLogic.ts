...
export function useEntityFormLogic(props: ViewEntityFormProps) {
  ...
  const updateTitle = useCallback(async (form?: EntityForm) => {
  ...
  const postSave = useCallback(async (entityForm: EntityForm): Promise<EntityForm> => {
  ...
  const postDelete = useCallback(async (entityForm: EntityForm): Promise<void> => {
    if (props.postDelete) {
  ...
  const openBaseLoadingCallback = useCallback((open: boolean) => {
  ...
  const handleCloseModal = useCallback(async (id: string) => {
    await closeModal(id);
  }, [closeModal]);
  ...
  const resetEntityForm = useCallback(async (delay?: number, preserveState: boolean = true) => {
    // 현재 상태 보존
