    ...
    props.errors.forEach((value, key) => {
      ...
      value.forEach((error, index) => {
        ...
        if (error.errors === undefined || error.errors.length === 0 || !Array.isArray(error.errors)) {
          errorMessageView.push(<div key={`error-message-${index}-undefined`}>
            {t('form.save.error.invalid')}
          </div>)
        } else {
          error.errors.forEach((message, i) => {
            errorMessageView.push(<div key={`error-message-${index}-${i}`}>
              {t(message)}
            </div>)
          });
        }

        tabError.push(<div key={`errorDetail${index}`} className={''}>
          <button
            type="button"
            onClick={() => handleFieldClick(error, key)}
            className={'flex space-x-2 items-center cursor-pointer hover:underline text-left w-full'}
          >
      ...
      errorView.push(<div className={'text-danger bg-danger-light dark:bg-danger-dark-light p-4 mb-2'} key={`error_${key}`}>
        <div className={'py-2 font-bold text-[16px] cursor-pointer hover:underline'} onClick={() => handleErrorClick(key)}>
          {t(key)}
        </div>
