...
export const ViewTabPanel = ({ id, tabIndex, entityForm, setEntityForm, readonly, subCollectionEntity, session, createStepFields, resetEntityForm, hideMappedByFields, ...props }: ViewTabPanelProps) => {
  ...
  if (groups.length === 0) {
    return <Tab.Panel className={classNames.tabPanel?.panel} unmount={false}><div className={cn('p-4 text-gray-500 dark:text-gray-400', classNames.tabPanel?.empty)}>이 단계에서는 표시할 내용이 없습니다.</div></Tab.Panel>
  }
          ...
          groups.forEach((group, index) => {
            panels.push(
              <div key={`${group}-${index}-${createStepFields ? createStepFields?.join(',') : ''}`}>
                <ViewFieldGroup tabId={id} groupId={group} key={index}
                  readonly={readonly}
                  subCollectionEntity={subCollectionEntity}
                  entityForm={entityForm} setEntityForm={setEntityForm}
                  session={session}
