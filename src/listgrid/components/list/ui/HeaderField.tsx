  ...
  return <React.Fragment>
    {listFields.map((field, index) => {
      if (viewFields.length === 0 || viewFields.includes(field.getName())) {
        ...
        return <th key={`th_${index}`} className={cellClass}>
          <div className={`w-full min-w-[40px] flex items-center ${alignClassName}`}>
            <span>{field.viewLabel(t)}</span>
            {(isTrue(sortable)) && <span className={`inline-flex items-center ${sortIconClass}`}>
              <SortField name={field.name} searchForm={props.searchForm} onChangeSearchForm={props.onChangeSearchForm}/></span>}
            {filterable && <span className={`inline-flex items-center ${filterIconClass}`}>
              <HeaderFieldFilter
                field={field}
                gridId={props.gridId}
    ...
    {draggable && <th key={`th_drag`} className={themeClasses.headerCell?.dragHandleCell ?? 'w-2 whitespace-nowrap'}>{t('순서 변경')}</th>}
  </React.Fragment>;
