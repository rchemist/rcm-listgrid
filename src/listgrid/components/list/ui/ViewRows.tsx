...
export const ViewRows = (props: ViewRowsProps) => {
    ...
    return <React.Fragment>
      <tr className={rowClass} key={`tr_${index}_${key}`}>
        {managePriority && <td className={dragHandleCellClass}>
          <IconBaselineDensityMedium className={'w-4 h-4'}/>
        </td>}
        {enableCheckItem && <td className={`${checkboxCellClass} whitespace-nowrap space-x-1`}>
          {(() => {
        ...
        {showOpenInNewWindowButton && <td className={`${openNewWindowCellClass} whitespace-nowrap`}>
          <Tooltip label={openInNewWindow?.tooltip || '새창에서 열기'}>
            <button
              className={'btn btn-outline-secondary btn-sm p-1 min-w-[32px] min-h-[32px] flex items-center justify-center'}
        ...
        {inlineExpansion && <td className={`${themeClasses.cell?.selectCell ?? ''} whitespace-nowrap`}>
        ...
        {showViewButton && !inlineExpansion && <td className={`${themeClasses.cell?.selectCell ?? ''} whitespace-nowrap`}>
          <Tooltip label={'상세 보기'}>
            <button
              className={'btn btn-outline-info btn-sm p-1 min-w-[32px] min-h-[32px] flex items-center justify-center'}
        ...
        {openInNewWindowEnabled && isAdmin && !showOpenInNewWindowButton && !isSearchMode && <td className={`${openNewWindowCellClass} whitespace-nowrap`}></td>}
        <ViewColumn {...props} clickAccordion={
          // Priority: inline expansion > accordion > undefined
          inlineExpansion ? toggleInlineExpansion :
          accordionMode ? toggleAccordion : undefined
        } viewMode={props.viewMode}/>
        {draggable && <td className={'whitespace-nowrap space-x-1'}>
          {function () {
            ...
            if (index < totalCount) {
              // 아래로 내리기
              sortButtons.push(<Tooltip label={`아래로 내리기`} key={`btn_down_${index}`}>
                <button className={'btn-outline-secondary'}
                        onClick={() => {
                          ...
                          items[index] = temp;
                          onDrag!(items);
                        }}><Icon icon="mingcute:arrow-down-circle-fill" className={`h-[24px] w-[24px]`}/>
                </button>
              </Tooltip>);
            }
            ...
            if (index > 0) {
              // 위로 올리기
              sortButtons.push(<Tooltip label={`위로 올리기`} key={`btn_up_${index}`}>
                <button className={'btn-outline-secondary'}
                        onClick={() => {
                          const items = [...sortableList];
                          const temp = items[index - 1];
      ...
      {showAccordion && <tr key={`row_accordion_${index}`}>
