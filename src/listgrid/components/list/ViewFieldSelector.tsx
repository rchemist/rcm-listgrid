...
export const ViewFieldSelector = ({viewFields, setViewFields, ...props}: ViewFieldSelectorProps) => {
    ...
    <Popover position={'bottom'} shadow={'md'}
      ...
      <Popover.Target>
        <Tooltip label={'원하는 필드만 표시할 수 있습니다.'}>
          <button className={'btn btn-outline-primary whitespace-nowrap'} disabled={props.disabled}>
            목록 설정
          </button>
        </Tooltip>
      </Popover.Target>
      <Popover.Dropdown>
        <div className={`w-full grid ${gridCols} gap-2 ${maxWidth} p-4`}>
          {fields.map(field => {
            ...
            return (
              <div className={'flex whitespace-nowrap space-x-2 mr-2'} key={`checkbox_${field.getName()}`}>
                <label className={'cursor-pointer !font-normal !text-sm text-gray-700 dark:text-gray-200'}>
                <input type={'checkbox'} className={'form-checkbox !w-4 !h-4'} checked={checked} onChange={() => syncViewFieldsToCache(field.getName())}/>
