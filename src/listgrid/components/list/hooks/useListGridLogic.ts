...
export const useListGridLogic = (props: ViewListGridProps): any => {
  ...
  const enableHandleData: boolean =
    manyToOne === undefined && !isTrue(props.options?.readonly);
  ...
  const [dataTransferConfig, setDataTransferConfig] = useState<DataTransferConfig | undefined>(undefined);
  ...
  const fetchData = useCallback((form?: SearchForm) => {
    setLoading(true);
    ...
    (async () => {
      ...
      const selectFieldsWithLoadOptions = listFields
        .filter((field): field is SelectField =>
  ...
  const setOptionsFilters = useCallback(async (entityForm: EntityForm, searchForm: SearchForm | undefined) => {
  ...
  const onChangeSearchForm = useCallback(async (entityForm: EntityForm, searchForm: SearchForm, reset?: boolean, resetPage?: boolean) => {
  ...
  const initializeSearchForm = useCallback(async (): Promise<SearchForm> => {
    ...
    let searchForm: SearchForm | undefined = isMainEntity
      ? await getSessionStorageObject(hashKey, (value: string) =>
        SearchForm.deserialize(value)
      )
      : undefined;
  ...
  const syncViewFieldsFromCache = useCallback(() => {
    const cached = getListFieldsFromCache(
      entityForm.getUrl(),
      props.options?.subCollection?.name
    );
      ...
      (async () => {
        await showConfirm({
          ...
          if (!isEmpty(result.errors)) {
            ...
            fetchData();
            setOpenBaseLoading(false);
            await props.options?.delete?.postDelete?.(
              entityForm,
              rows,
              [...checkedItems]
            );
  ...
  const onSelect = (
    ...
    (isSubCollection
      ...
      : undefined)
  ) as (item: any, setManagedId?: (value: any) => void, clearFilterAndSort?: () => void) => void;
