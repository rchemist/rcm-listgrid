...
import {DataField, DataRow, DataRowSet, getExportFileName, SampleDataItem} from '../transfer/Type';
    ...
    <>
      ...
      {<div>
        {exportable && (<>
          {isBlank(error) && (
            <>
              {exportProgress < 100 && (<div className={'py-1 mb-3'}>
                <div className={'text-primary'}>
                  {t('form.list.dataTransfer.tab.export.onProcessing')}
                </div>
                <LinearIndicator value={exportProgress} key={'progress_indicator_' + exportProgress} />
              </div>)}
              {exportProgress >= 100 && (<div className={'py-1 mb-3'}>
          ...
          {!isBlank(error) && (<div className={'py-1 mb-3'}>
            <span className={'text-danger'}>
              {t(error)}
            </span>
          </div>)}
          {onDownload && <div className={'relative'}>
            <LoadingOverlay visible={true} />
            <div className={'w-full h-[100px]'}></div>
          </div>}
          {!onDownload && <div className={'flex items-center justify-center space-x-2'}>
            <Button variant={'filled'} onClick={() => {
              ...
              style={{ marginRight: 1 }}>{t('form.list.dataTransfer.tab.export.button.download')}</Button>
            <Button variant={'outline'} onClick={() => {
              setExportProgress(0);
              onProcessed();
            }}>{t('form.list.dataTransfer.tab.export.button.cancel')}</Button>
          </div>}
        </>)}
        {!exportable && (<>
          <div>
            <h3>{t('form.list.dataTransfer.tab.export.error.limit.retry')}</h3>
            <div>{t('form.list.dataTransfer.tab.export.error.limit.cause')}: {failedCount}</div>
            <div>{t('form.list.dataTransfer.tab.export.error.limit.warning')}: {maxLimitCount}</div>
          </div>
          <div className={'text-center'}>
            <Button variant={'outline'} color={'inherit'} onClick={() => {
              onProcessed();
            }}>{t('form.list.dataTransfer.tab.export.button.close')}</Button>
          </div>
        </>)}
      </div>}
    </>
