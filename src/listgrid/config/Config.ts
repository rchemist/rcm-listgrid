...
export type FieldType =
  'text' | 'number' | 'date' | 'datetime' | 'boolean' | 'select' | 'manyToOne' | 'email' | 'phone' | 'time'
  | 'month'
  | "tag"
  | 'file'
  | 'year'
...
export async function getConditionalBoolean(props: ConditionalValue, condition: ConditionalBooleanValue | undefined): Promise<boolean> {
  ...
  const renderTypeValue: RenderType | undefined = props.renderType ?? props.entityForm?.getRenderType();
...
export async function getConditionalString(props: ConditionalValue, condition: ConditionalStringValue | undefined): Promise<string> {
  if (!condition) {
    return '';
  }
...
export async function getConditionalReactNode(props: ConditionalValue, condition: ConditionalReactNodeValue | undefined): Promise<ReactNode> {
  if (!condition) {
    return '';
  }
...
export const NO_FILTER_SORT_ON_LIST: IListConfig = {support: true, sortable: false, filterable: false};
...
export async function onChangeNameToSlug(entityForm: EntityForm, fieldName: string, targetFieldName: string) {
...
export function checkDuplicateValueProcess({url, fieldName, label, ...props}: CheckButtonValidationFieldProps) {
